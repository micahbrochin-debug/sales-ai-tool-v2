...
function StakeholderRolesView({ roleAnalysis }: { roleAnalysis: RoleAnalysis[] }) {
  ...
  const getRoleColor = (role: StakeholderRole) => {
    switch (role) {
      case 'economic buyer': return 'bg-yellow-100 text-yellow-800';
  ...
  return (
    <div className="space-y-6">
      {Object.entries(groupedRoles).map(([role, people]) => {
        ...
        return (
          <div key={role} className="card">
            <div className="flex items-center space-x-3 mb-4">
              <RoleIcon className="text-primary-600" size={24} />
              <h4 className="text-lg font-semibold text-secondary-900 capitalize">
                {role.replace('_', ' ')}
              </h4>
              <span className={`px-3 py-1 text-sm rounded-full ${roleColor}`}>
                {people.length}
              </span>
            </div>
              ...
              {people.map((person, index) => (
                <div key={index} className="p-3 bg-secondary-50 rounded-md">
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <h5 className="font-medium text-secondary-900">{person.name}</h5>
                      <p className="text-sm text-secondary-600">{person.title}</p>
                    </div>
                  </div>
                  
                  {person.notes && (
                    <p className="text-sm text-secondary-700 mb-2">{person.notes}</p>
                  )}
                  
                  <div className="flex flex-wrap gap-1">
                    {person.sources.map((source, sourceIndex) => (
                      <a
                        key={sourceIndex}
                        href={source}
                        target="_blank"
